...
# UI Layout: Split into two columns
col1, col2 = st.columns([2, 3])  # Adjust the width for input and chart

# Left Column: Input Sliders & Calculation
with col1:
    ...
    # Compute Investability Index Manually
    input_values = [
...
# Right Column: Radar Chart (Enhanced)
with col2:
    ...
    def plot_radar_chart(scores):
        ...
        fig, ax = plt.subplots(figsize=(4, 4), subplot_kw=dict(polar=True))  # Reduced size
        ax.fill(angles, values, color="dodgerblue", alpha=0.3)
        ...
        ax.set_title("📊 Startup Strengths & Weaknesses", fontsize=10, fontweight="bold")

        st.pyplot(fig)

    plot_radar_chart(input_values)

# 📌 Insights Section: Dynamic Parameter-Specific Recommendations
...
# Display insights for each parameter
