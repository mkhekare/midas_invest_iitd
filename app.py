...
# UI Layout: Fixed one-window design using st.columns
col1, col2 = st.columns([1.5, 1.5])  # Adjust column widths
...
# Right Column: Radar Chart (Enhanced Design)
with col2:
    ...
    def plot_radar_chart(scores):
        ...
        fig, ax = plt.subplots(figsize=(3.5, 3.5), subplot_kw=dict(polar=True))  # Reduced size
        ...
        ax.set_title("Startup Strengths & Weaknesses", fontsize=10, fontweight="bold", pad=15)  # Adjust title placement

        st.pyplot(fig)

    plot_radar_chart(input_values)

# Below: Dynamic Recommendations Section
